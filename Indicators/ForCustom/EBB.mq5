...
//|                                                          EBB.mq5 |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025-2025, shabaraba"
#property link "https://shaba.dev"
#property description "Exponential Bollinger Bands"
...
void OnInit() {
  ...
  if (InpBandsPeriod < 2) {
    ExtBandsPeriod = 20;
    PrintFormat("Incorrect value for input variable InpBandsPeriod=%d. "
  ...
  if (InpBandsShift < 0) {
    ExtBandsShift = 0;
    PrintFormat("Incorrect value for input variable InpBandsShift=%d. "
                "Indicator will use value=%d for calculations.",
  ...
  if (InpBandsDeviations == 0.0) {
    ...
    PrintFormat("Incorrect value for input variable InpBandsDeviations=%f. "
                "Indicator will use value=%f for calculations.",
  ...
  PlotIndexSetString(0, PLOT_LABEL,
                     "Bands(" + string(ExtBandsPeriod) + ") Middle");
  PlotIndexSetString(1, PLOT_LABEL,
                     "Bands(" + string(ExtBandsPeriod) + ") Upper");
  PlotIndexSetString(2, PLOT_LABEL,
                     "Bands(" + string(ExtBandsPeriod) + ") Lower");
  //--- indicator name
...
int OnCalculate(const int rates_total, const int prev_calculated,
  ...
  for (int i = pos; i < rates_total && !IsStopped(); i++) {
    //--- middle line
    ExtMLBuffer[i] =
        i == 0 ? SimpleMA(i, ExtBandsPeriod, price)
               : ExponentialMA(i, ExtBandsPeriod, ExtMLBuffer[i - 1], price);
...
//+------------------------------------------------------------------+
double StdDev_Func(const int position, const double &price[],
                   const double &ma_price[], const int period) {
  double std_dev = 0.0;
  //--- calcualte StdDev
  if (position >= period) {
